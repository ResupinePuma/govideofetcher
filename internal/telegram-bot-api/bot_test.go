...
func TestCopyMessage(t *testing.T) {
	...
	copyMessageConfig := NewCopyMessage(SupergroupChatID, message[0].Chat.ID, message[0].MessageID)
	messageID, err := bot.CopyMessage(copyMessageConfig)
	if err != nil {
		t.Error(err)
	}

	if messageID.MessageID == message[0].MessageID {
...
func TestUnpinAllChatMessages(t *testing.T) {
	...
	unpinAllChatMessagesConfig := UnpinAllChatMessagesConfig{
		ChatConfig: ChatConfig{ChatID: message[0].Chat.ID},
	}
...
func TestPolls(t *testing.T) {
	...
	result, err := bot.StopPoll(NewStopPoll(SupergroupChatID, msg[0].MessageID))
	if err != nil {
		t.Error(err)
	}
